...
globalThis.runTrackTransportTests = async function (sourceCode) {
    ...
    function harness(enabled = true) {
        ...
        let serial = 0, now = 0;
        const labels = {green:'green',yellow:'yellow',red:'red',unknown:'unknown',empty:'empty',connected:'connected',reconnecting:'reconnecting',stale:'stale',unavailable:'unavailable',loading:'loading'};
    ...
    assert(h.nodes.get('track-label').textContent==='green','GET renders GREEN');
    const stream=h.streams[0];stream.emit('snapshot',yellow);
    assert(h.nodes.get('track-label').textContent==='yellow','SSE renders yellow without reload');
    ...
    assert(h.nodes.get('track-label').textContent==='yellow','Disconnect keeps last flag');
    h.setNow(40000);h.tick();
    assert(h.nodes.get('track-label').textContent==='yellow'&&h.nodes.get('sector-time-32').textContent==='00:00','Expiry during disconnect never fabricates GREEN');
    ...
    assert(h.nodes.get('track-label').textContent==='yellow','Heartbeat alone cannot clear disconnected flags');
    resumed.emit('snapshot',{...base,server_time:'2026-09-09T12:02:00Z'});
    assert(h.nodes.get('track-label').textContent==='green'&&!h.nodes.get('track-yellows').children.length,'Reconnect snapshot fully replaces state');
    ...
    assert(h.nodes.get('track-label').textContent==='yellow','Invalid snapshot preserves last known state');
    ...
    fail.streams[0].emit('snapshot',base);assert(fail.nodes.get('track-label').textContent==='green','SSE recovery after GET failure');
    ...
    assert(late.nodes.get('track-label').textContent==='yellow'&&late.streams.length===1,'Late GET cannot overwrite restored SSE');
